...
public class AudioService extends Service {
	...
	@Subscribe
	public void onEvent(AudioVolumeEvent event){
	...
	public void onDestroy() {
		...
		EventBus.getDefault().post(new AudioServiceCallBack(-1,-1,null, last_action));
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		case ACTION_PAUSE:
			...
			EventBus.getDefault().post(new AudioServiceCallBack(currentposplay,sources.length,sources[currentposplay], last_action));
			break;
		case ACTION_RESUME:
			isPlaying = true;
			if (mp != null && !mp.isPlaying())
	...
	private void playSong(final String songPath) {
		...
		mp.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
			...
			public void onPrepared(MediaPlayer mp) {
				mp.start();
				EventBus.getDefault().post(new AudioServiceCallBack(currentposplay,sources.length,songPath, ACTION_START));
			}
		});
	...
	public static void startAudioSound(Context ctx, String[] sources) {
	...
	public static void pauseAudioSound(Context ctx) {
	...
	public static void resumeAudioSound(Context ctx) {
		if (AudioService.isRunning()) {
			Intent in = new Intent(ctx, AudioService.class);
	...
	public static void stopAudioSound(Context ctx) {
